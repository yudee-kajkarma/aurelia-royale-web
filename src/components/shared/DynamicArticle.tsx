  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16 md:py-24">
      {sections.map((section, idx) => {
        const hasFaqBlock = section.content.some((b) => b.type === "faq");
        const isFaqHeading =
          section.heading &&
        ...
        // Hide outer section.heading if this section contains an FAQ block to avoid duplicate title
        const showHeading = section.heading && !(hasFaqBlock && isFaqHeading);
                  ...
                  return (
                    <div key={bIdx} className="mt-20 bg-[#153f35] text-[#efefe8] p-10 rounded-lg text-center shadow-2xl relative overflow-hidden">
                      ...
                      <div className="flex flex-col sm:flex-row justify-center gap-4">
                        <Link
                          href={localizeHref(block.shopHref)}
                          className="inline-block bg-gold text-[#031b16] font-jost text-xs font-semibold uppercase tracking-[0.2em] px-8 py-4 rounded hover:bg-gold/90 transition-colors duration-300"
                        >
                          {t("blog.exploreCollection")}
                        </Link>
                        <Link
                          href={localizeHref(block.contactHref)}
