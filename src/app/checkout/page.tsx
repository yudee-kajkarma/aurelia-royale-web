    ...
    <AuthGuard allowedRoles={["USER", "ADMIN"]}>
      <main className="mx-auto min-h-[70vh] w-full max-w-[1480px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.22em] text-[var(--foreground)]/45">Checkout</p>
            <h1 className="display-font mt-3 text-4xl text-[var(--deep)] sm:text-5xl">Review and Place Your Order</h1>
          ...
          <Link
            href="/cart"
            className="inline-flex items-center justify-center rounded-full border border-[var(--gold)]/35 bg-white px-5 py-3 text-sm font-bold uppercase tracking-[0.08em] text-[var(--deep)] transition hover:border-[var(--deep)] hover:bg-[var(--deep)] hover:text-white"
          >
            Back to Cart
          </Link>
        </div>
        ...
        <div className="mt-8 grid gap-8 xl:grid-cols-[1fr_380px]">
          <section className="space-y-6 rounded-[34px] border border-[var(--foreground)]/10 bg-white/90 p-6 shadow-[0_20px_60px_rgba(55,31,10,0.06)] backdrop-blur-sm sm:p-8">
            {isLoading ? <p className="text-sm font-medium text-[var(--foreground)]/60">Loading checkout...</p> : null}

            {!isLoading && items.length === 0 ? (
              <div className="rounded-[28px] border border-dashed border-[var(--gold)]/35 bg-[var(--surface)] px-6 py-12 text-center">
                <h2 className="display-font text-3xl text-[var(--deep)]">Nothing to checkout yet</h2>
            ...
            {items.length > 0 ? (
              <>
                <div className="rounded-[28px] border border-[var(--foreground)]/10 bg-[var(--surface)] p-5">
                  ...
                  <div className="mt-5 space-y-4">
                    {items.map((item) => (
                      ...
                      >
                        ...
                        <div>
                          <h3 className="text-lg font-bold text-[var(--deep)]">{item.title}</h3>
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  <div className="rounded-[28px] border border-[var(--foreground)]/10 bg-[var(--surface)] p-5">
                    ...
                    <div className="mt-5 rounded-[22px] border border-[var(--foreground)]/10 bg-white p-4 text-sm leading-7 text-[var(--foreground)]/68">
                      <p className="font-semibold text-[var(--deep)]">{user?.username ?? "Dalia Customer"}</p>
                  ...
                  <div className="rounded-[28px] border border-[var(--foreground)]/10 bg-[var(--surface)] p-5">
                    ...
                    <div className="mt-5 space-y-3">
                      <label className={`flex cursor-pointer items-start gap-4 rounded-[22px] border px-4 py-4 transition ${paymentMethod === "ONLINE" ? "border-[var(--deep)] bg-white" : "border-[var(--foreground)]/10 bg-white/70 hover:border-[var(--gold)]/45"}`}>
                        <input
                          type="radio"
                          name="paymentMethod"
                          value="ONLINE"
                          ...
                          className="mt-1 h-4 w-4 border-[var(--foreground)]/20 text-[var(--deep)]"
                        />
                        <div>
                          <p className="font-bold text-[var(--deep)]">Online payment</p>
                          <p className="mt-1 text-sm leading-6 text-[var(--foreground)]/60">A Stripe checkout popup opens after order creation and returns to the status page.</p>
                        </div>
                      </label>
                      <label className={`flex cursor-pointer items-start gap-4 rounded-[22px] border px-4 py-4 transition ${paymentMethod === "COD" ? "border-[var(--deep)] bg-white" : "border-[var(--foreground)]/10 bg-white/70 hover:border-[var(--gold)]/45"}`}>
                        <input
                          type="radio"
                          name="paymentMethod"
                          value="COD"
                        ...
                        />
                        <div>
                          <p className="font-bold text-[var(--deep)]">Cash on delivery</p>
                          <p className="mt-1 text-sm leading-6 text-[var(--foreground)]/60">The order is created immediately and you are redirected to the local success screen.</p>
                        </div>
                      </label>
                    </div>
                  </div>
          ...
          <aside className="rounded-[34px] border border-[var(--foreground)]/10 bg-white/90 p-6 shadow-[0_20px_60px_rgba(55,31,10,0.06)] backdrop-blur-sm sm:p-8">
            <p className="text-sm font-bold uppercase tracking-[0.22em] text-[var(--foreground)]/45">Order Summary</p>
            ...
            <div className="mt-6 rounded-[24px] border border-[var(--gold)]/20 bg-[var(--surface)] p-4">
              <div className="flex items-start gap-3 text-sm leading-6 text-[var(--foreground)]/62">
                <ShieldCheck className="mt-0.5 h-4 w-4 text-[var(--deep)]" />
                <p>
            ...
            <div className="mt-6 border-t border-[var(--foreground)]/10 pt-6">
              ...
              <button
                ...
                className="cta-sweep mt-6 inline-flex w-full items-center justify-center border border-[var(--deep)] bg-[var(--deep)] px-6 py-4 text-sm font-bold uppercase tracking-[0.08em] text-white transition hover:border-[var(--gold)] hover:text-[var(--deep)] focus-visible:border-[var(--gold)] focus-visible:text-[var(--deep)] disabled:opacity-60 hover:!text-white"
              >
