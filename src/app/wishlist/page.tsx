    ...
    <AuthGuard allowedRoles={["USER", "ADMIN"]}>
      <main className="mx-auto min-h-[70vh] w-full max-w-[1480px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.22em] text-[var(--foreground)]/45">Wishlist</p>
            <h1 className="display-font mt-3 text-4xl text-[var(--deep)] sm:text-5xl">Saved Pieces You Love</h1>
        ...
        <div className="mt-8 grid gap-8 xl:grid-cols-[1fr_360px]">
          <section className="rounded-[34px] border border-[var(--foreground)]/10 bg-white/90 p-6 shadow-[0_20px_60px_rgba(55,31,10,0.06)] backdrop-blur-sm sm:p-8">
            {isLoading ? <p className="text-sm font-medium text-[var(--foreground)]/60">Loading wishlist...</p> : null}

            {!isLoading && count === 0 ? (
              <div className="rounded-[28px] border border-dashed border-[var(--gold)]/35 bg-[var(--surface)] px-6 py-12 text-center">
                <h2 className="display-font text-3xl text-[var(--deep)]">Your wishlist is empty</h2>
              ...
              <div className="space-y-5">
                {items.map((item) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--foreground)]/45">Saved Collection</p>
                      <p className="mt-2 text-xl font-semibold text-[var(--deep)]">{item.title}</p>
                      <p className="mt-2 text-sm text-[var(--foreground)]/55">Saved {new Date(item.addedAt).toLocaleDateString()}</p>
                    ...
                    <div className="flex flex-col items-start gap-3 sm:items-end">
                      <Link
                        href="/shop"
                        className="cta-sweep inline-flex items-center justify-center border border-[var(--deep)] bg-[var(--deep)] px-5 py-3 text-xs font-bold uppercase tracking-[0.1em] text-white transition hover:border-[var(--gold)] hover:text-[var(--deep)] focus-visible:border-[var(--gold)] focus-visible:text-[var(--deep)] hover:!text-white"
                      >
          ...
          <aside className="rounded-[34px] border border-[var(--foreground)]/10 bg-white/90 p-6 shadow-[0_20px_60px_rgba(55,31,10,0.06)] backdrop-blur-sm sm:p-8">
            ...
            <div className="mt-6 space-y-4 text-sm text-[var(--foreground)]/68">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-[var(--deep)]">{count > 0 ? "Yes" : "No"}</span>
              </div>
            </div>
            ...
            <div className="mt-6 border-t border-[var(--foreground)]/10 pt-6">
              <Link
                href="/shop"
                className="cta-sweep inline-flex w-full items-center justify-center border border-[var(--deep)] bg-[var(--deep)] px-6 py-4 text-sm font-bold uppercase tracking-[0.08em] text-white transition hover:border-[var(--gold)] hover:text-[var(--deep)] focus-visible:border-[var(--gold)] focus-visible:text-[var(--deep)] hover:!text-white"
              >
