...
export default function AdminWishlistsPage() {
  const [users, setUsers] = useState<AdminWishlistUserSummary[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        const wishlist = await adminWishlistService.getAdminUserWishlist(nextSelectedUserId);
  ...
  const summary = useMemo(() => {
    return users.reduce(
      (accumulator, user) => ({
        ...
        totalValue: accumulator.totalValue + user.totalValue,
      }),
  ...
  async function handleSelectUser(userId: string) {
    ...
    try {
      const wishlist = await adminWishlistService.getAdminUserWishlist(userId);
      setSelectedWishlist(wishlist);
    } catch (error) {
      setSelectedWishlist(null);
        ...
        <section className="rounded-[28px] border border-[var(--foreground)]/10 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--gold)]">Admin Wishlists</p>
              <h1 className="display-font mt-3 text-3xl text-[var(--foreground)] sm:text-4xl">Wishlist Oversight Workspace</h1>
              <p className="mt-3 text-sm leading-7 text-[var(--foreground)]/65 sm:text-base">
                Review users saving products, inspect wishlist items, and track saved-value demand in the same admin theme used across the management workspace.
              </p>
            </div>
          ...
          <div className="mt-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[34rem] xl:flex-1">
              ...
              <div className="rounded-[22px] border border-[var(--foreground)]/10 bg-[var(--surface)] px-4 py-4">
                <p className="text-xs uppercase tracking-[0.16em] text-[var(--foreground)]/45">Wishlist Value</p>
                <p className="mt-2 text-2xl font-semibold text-[var(--foreground)]">{formatCurrency(summary.totalValue)}</p>
              </div>
            </div>
        ...
        <div className="mt-5 grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
          <section className="rounded-[28px] border border-[var(--foreground)]/10 bg-white p-4 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3 border-b border-[var(--foreground)]/10 px-1 pb-4">
              <div>
                <p className="text-sm font-semibold text-[var(--foreground)]">Users With Wishlists</p>
                <p className="mt-1 text-sm text-[var(--foreground)]/60">
                  {pagination ? `Page ${pagination.currentPage} of ${pagination.totalPages}` : "Current wishlist owners"}
                </p>
              </div>
            </div>
            ...
            {!loadingUsers && users.length === 0 ? (
              <div className="rounded-[22px] border border-dashed border-[var(--foreground)]/10 bg-[var(--surface)] px-5 py-10 text-center text-sm text-[var(--foreground)]/60">
                No users with wishlists matched the current search.
              </div>
            ) : null}
              ...
              <div className="space-y-3">
                {users.map((wishlistUser) => (
                  <button
                    ...
                    type="button"
                    onClick={() => void handleSelectUser(wishlistUser.userId)}
                    className={selectedUserId === wishlistUser.userId
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-semibold text-[var(--foreground)]">{wishlistUser.userName || "Unknown user"}</p>
                        <p className="mt-1 truncate text-xs text-[var(--foreground)]/58">{wishlistUser.userEmail || "--"}</p>
                        <p className="mt-1 text-xs text-[var(--foreground)]/58">{wishlistUser.userPhone || "Phone not provided"}</p>
                      </div>
                      <span className="rounded-full border border-[var(--foreground)]/10 bg-white px-3 py-1 text-xs font-semibold text-[var(--foreground)]">
                        {wishlistUser.itemCount} item{wishlistUser.itemCount === 1 ? "" : "s"}
                      </span>
                    </div>

                    <div className="mt-4 flex items-center justify-between gap-3 text-xs text-[var(--foreground)]/58">
                      <span className="font-semibold text-[var(--gold)]">{formatCurrency(wishlistUser.totalValue)}</span>
                      <span>Updated {formatDateLabel(wishlistUser.lastUpdated)}</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {pagination ? (
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--foreground)]/10 px-1 pt-4 text-sm text-[var(--foreground)]/60">
                <span>{pagination.totalRecords} wishlist user{pagination.totalRecords === 1 ? "" : "s"}</span>
          ...
          <section className="rounded-[28px] border border-[var(--foreground)]/10 bg-white p-5 shadow-sm">
            ...
            {!loadingWishlist && !selectedWishlist ? (
              <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[22px] border border-dashed border-[var(--foreground)]/10 bg-[var(--surface)] px-6 text-center">
                ...
                <p className="mt-4 text-sm font-semibold text-[var(--foreground)]">Select a user to inspect their wishlist.</p>
              </div>
            ) : null}
            ...
            {!loadingWishlist && selectedWishlist ? (
              <>
                <div className="rounded-[22px] border border-[var(--foreground)]/10 bg-[var(--surface)] p-5">
                  <p className="text-xs uppercase tracking-[0.16em] text-[var(--foreground)]/45">Wishlist Overview</p>
                  <h2 className="mt-2 text-2xl font-semibold text-[var(--foreground)]">
                    {selectedWishlist.userDetails?.userName || "Unknown user"}
                  </h2>
                  <p className="mt-1 text-sm text-[var(--foreground)]/60">{selectedWishlist.userDetails?.userEmail || "--"}</p>
                  <p className="mt-1 text-sm text-[var(--foreground)]/60">{selectedWishlist.userDetails?.userPhone || "Phone not provided"}</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3 text-xs text-[var(--foreground)]/58">
                    <span className="rounded-full border border-[var(--foreground)]/10 bg-white px-3 py-1">
                      Updated {formatDateLabel(selectedWishlist.updatedAt)}
                    </span>
                    <span className="rounded-full border border-[var(--foreground)]/10 bg-white px-3 py-1">
                      {selectedWishlist.items.length} item{selectedWishlist.items.length === 1 ? "" : "s"}
                    </span>
                    <span className="rounded-full border border-[var(--foreground)]/10 bg-white px-3 py-1">
                      {selectedWishlist.isActive ? "Active wishlist" : "Inactive wishlist"}
                    </span>
                  </div>
                </div>

                <div className="mt-5 flex items-center justify-between gap-3">
                  <h3 className="text-lg font-semibold text-[var(--foreground)]">Wishlist Items</h3>
                  <span className="rounded-full border border-[var(--foreground)]/10 bg-[var(--surface)] px-3 py-1 text-xs font-semibold text-[var(--foreground)]">
                    Total {formatCurrency(selectedWishlist.items.reduce((total, item) => total + item.price, 0))}
                  </span>
                </div>
                ...
                {selectedWishlist.items.length === 0 ? (
                  <div className="mt-4 rounded-[22px] border border-dashed border-[var(--foreground)]/10 bg-[var(--surface)] px-5 py-10 text-center text-sm text-[var(--foreground)]/60">
                    This wishlist does not contain any items.
                  </div>
                ) : (
                    ...
                    {selectedWishlist.items.map((item, index) => (
                      ...
                      >
                        <div className="h-20 w-20 shrink-0 overflow-hidden rounded-[18px] border border-[var(--foreground)]/10 bg-[var(--surface)] p-2">
                          {item.thumbnail ? (
                            <ProductImage src={item.thumbnail} alt={item.title || "Wishlist product"} className="h-full w-full object-contain" />
                          ) : (
                        ...
                        <div className="text-right text-sm">
                          <p className="font-semibold text-[var(--gold)]">{formatCurrency(item.price)}</p>
                        </div>
                      </article>
                    ))}
                  </div>
