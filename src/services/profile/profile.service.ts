...
function normalizeUserAddress(value: unknown): UserAddress | null {
  ...
  return {
    id: getStringValue(record, "_id") || getStringValue(record, "id"),
    ...normalized,
  };
}
...
function normalizeUserProfile(data: unknown): UserProfile {
  ...
  const userRecord = userRecordRaw && typeof userRecordRaw === "object" && !Array.isArray(userRecordRaw)
    ? (userRecordRaw as Record<string, unknown>)
    : dataRecord;
  ...
  return {
    id: getStringValue(userRecord, "_id") || getStringValue(userRecord, "id"),
...
async function updateUserProfile(payload: UpdateProfileFormPayload): Promise<{ message: string }> {
  ...
  try {
    const response = await apiClient.post<ApiEnvelope<unknown>>("/users/update-profile", requestPayload);
    return {
...
async function getUserProfile(): Promise<UserProfile> {
  try {
    const response = await apiClient.get<ApiEnvelope<unknown>>("/users/profile");
...
async function getUserAddresses(): Promise<UserAddress[]> {
  try {
    const response = await apiClient.get<ApiEnvelope<unknown>>("/addresses");
...
async function createUserAddress(payload: UserProfileAddressPayload): Promise<UserAddress> {
  try {
    const response = await apiClient.post<ApiEnvelope<unknown>>("/addresses", payload);
    const dataRecord = response.data.data && typeof response.data.data === "object" && !Array.isArray(response.data.data)
      ? (response.data.data as Record<string, unknown>)
      : {};
...
async function updateUserAddress(addressId: string, payload: Partial<UserProfileAddressPayload>): Promise<void> {
  try {
    await apiClient.put(`/addresses/${addressId}`, payload);
  } catch (error) {
    throw new Error(getApiErrorMessage(error));
  }
}

async function deleteUserAddress(addressId: string): Promise<void> {
  try {
...
async function setDefaultUserAddress(addressId: string): Promise<void> {
  try {
    await apiClient.patch(`/addresses/${addressId}/set-default`);
  } catch (error) {
    throw new Error(getApiErrorMessage(error));
  }
}
